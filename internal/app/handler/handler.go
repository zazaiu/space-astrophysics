...
// 3. ЗАЯВКА
func (h *Handler) ViewMission(ctx *gin.Context) {
	idStr := ctx.Param("id")
	id, _ := strconv.Atoi(idStr)

	order, err := h.Repository.ViewMissionByID(id)
	if err != nil {
		ctx.String(http.StatusNotFound, err.Error())
		return
	}
...
func (h *Handler) AddPlanet(ctx *gin.Context) {
	...
	// если планета уже есть в заказе — увеличиваем количество
...
func (h *Handler) CalcOrder(ctx *gin.Context) {
	...
	// считаем средний угол по всем планетам в заказе
...
// 7. Очистить заявку
func (h *Handler) ClearPlanet(ctx *gin.Context) {
	idStr := ctx.Param("id")
	id, _ := strconv.Atoi(idStr)
	// просто заказ пустой
