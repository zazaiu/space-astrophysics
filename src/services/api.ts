...
// Интерфейс для фронтенда (только поля из бэкенда)
export interface Planet {
  id: number;
  name: string;
  description: string;
  image: string;
  distance: string; // вычисляем из A
  orbitalPeriod: string; // из Period
  characteristics: Record<string, string>;
}

// Функция для преобразования данных с бэкенда в формат фронтенда
export const adaptBackendPlanet = (backendPlanet: BackendPlanet): Planet => {
...
export const fetchPlanets = async (): Promise<Planet[]> => {
  try {
    ...
    const backendPlanets: BackendPlanet[] = await response.json();
...
export const fetchPlanetById = async (id: number | string): Promise<Planet | null> => {
  try {
    ...
    const backendPlanet: BackendPlanet = await response.json();
...
const getMockPlanets = (): Planet[] => {
  const mockBackendPlanets: BackendPlanet[] = [
    {
      ID: 1,
      Name: "Меркурий",
...
export const getPlanetImage = (imageName: string): string => {
...
export const getPlanetImageById = (planetId: number): string => {
